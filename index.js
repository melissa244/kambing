...
(async () => {
    ...
    while (true) {
        try {
            ...
            const createspotify = await spotify(`birth_day=11&name=${cacah[0]}&gender=neutral&birth_year=1995&platform=Android-ARM&email=${email}&creation_point=client_mobile&password=AripGanteng69&password_repeat=AripGanteng69&iagree=true&birth_month=4&key=142b583129b2df829de3656f9eb484e6`);
                ...
                for (i = 0; i < 5; i++) {
                    try {
                        ...
                        if (cekemail == undefined) {
                            ...
                            fs.appendFileSync('setoran.txt', `${email}|AripGanteng69\n`);
                            break;
                        }
                    } catch (error) {
                        console.log(`[${a}] Gagal Konfirmasi Email\n`);
                        fs.appendFileSync('ampas.txt', `${email}|AripGanteng69\n`);
                    }
                }
            } else {
